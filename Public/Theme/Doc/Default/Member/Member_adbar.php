<?php if ($this->session()->get('doc')['member_id'] == '1'): ?>
    ...
    <div class="super-sidebar">
        <?php foreach (
            ...
            <script>
                $(function () {
                    ...
                    $.getJSON('<?= PESCMS_URL ?>/?g=Api&m=<?= $item['api'] ?>&a=recommend&project=3', function (res) {
        ...
        <div class="am-panel am-panel-default">
            <div class="am-panel-bd am-margin-bottom">
                <span class="am-fl"><strong>PESCMS Doc最近动态<span class="am-text-xs am-text-danger"> [仅超级管理员可见]</span></strong></span>
            ...
            <table class="pes-article am-table am-table-hover am-text-xs">
                <tr>
                    <td><i class="am-icon-spinner am-icon-spin"></i> 正在获取Doc最新动态...</td>
                </tr>
            </table>
        </div>
        <script>
            $(function () {
                ...
                //获取PESCMS Ticket最新动态
                $.getJSON('https://www.pescms.com/?g=Api&m=Article&a=index&type=3', function (res) {
                    var status = res.status;
                    if (status == 200) {
                        let pesArtciel = '';
                        for (var i in res.data) {
