        ...
        <ul class="am-nav am-nav-pills am-topbar-nav">
            <?php foreach ($menu as $item): ?>
                <?php if (empty($item['child'])): ?>
                    <li>
                        <a href="<?= empty($item['node_link']) ? 'javascript:;' : ($item['node_link_type'] == 0 ? $label->url($item['node_link']) : $item['node_link']) ?>"><i class="<?= $item['node_menu_icon'] ?>"></i> <?= $item['node_name'] ?></a>
                    </li>
                <?php else: ?>
                    <li class="am-dropdown" >
                        <a class="am-dropdown-toggle" href="javascript:;">
                            <i class="<?= $item['node_menu_icon'] ?>"></i> <?= $item['node_name'] ?> <span class="am-icon-caret-down"></span>
                        </a>
                        <div class="am-dropdown-layer">
                            <ul class="am-dropdown-content">
                                <?php foreach($item['child'] as $key => $value): ?>
                                    <li><a href="<?= $value['node_link_type'] == 0 ? $label->url($value['node_link']) : $value['node_link'] ?>"><i class="<?= $value['node_menu_icon'] ?>"></i> <?= $value['node_name'] ?></a></li>
                                <?php endforeach; ?>
                            </ul>
                        </div>
                    </li>
