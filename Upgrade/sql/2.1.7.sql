...
(NULL, 'api_status', 'API状态', '0', 'API设置', 'api', 'string', 'radio', '{\"关闭\":\"0\",\"开启\":\"1\"}', 1, '', 1),
...
), 'API设置', 'api', 'string', 'text', '', 1, '', 2),
(NULL, 'api_secret', 'API Secret', SUBSTRING(MD5(RAND()), 1, 32), 'API设置', 'system', 'string', 'text', '', 1, '', 3);

UPDATE `pes_field` SET `field_required` = '1' WHERE `field_model_id` = 5 AND `field_name` = 'createtime';